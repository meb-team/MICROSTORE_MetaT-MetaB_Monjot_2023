...
#
  ...
  ## plot
    ggplot(ASV_stat_rare_FractionXZone_melt, aes(x = Abundance)) + facet_grid(.~Fraction) + 
      ...
      scale_y_continuous() +
      scale_x_continuous(trans = log2_trans(),breaks = w) +
      labs(y="ASVs") + geom_vline(xintercept = sum(colSums(seq_mat_rare %>% select(-Taxonomy)))*0.00002, linetype=2,color = "darkgrey", linewidth=0.5) +
      theme(legend.title = element_text(face="bold"),
            axis.title = element_text(color = "black", face = "bold"),
            strip.text.x = element_text(color = "black", face = "bold", size = 12),
...
# ANOSIM in periods X Zone
  # Mixolimnion
  dtMixo <- dt %>% filter(Zone=="Mixolimnion")
  dtMixox <- dtMixo %>% select(-Cycle,-Period, -Fraction, -Zone, -Row.names)
  distMixo <- vegdist(dtMixox, method ="bray")
  PeriodsXMixolimnion_anosim <- anosim(distMixo, dtMixo$Period, permutations = 9999, parallel = 10)
  capture.output(PeriodsXMixolimnion_anosim,file="Betadisp/anosim_PeriodsinMixolimnion_seq.txt")
  # Monimolimnion
  dtMoni <- dt %>% filter(Zone=="Monimolimnion")
  dtMonix <- dtMoni %>% select(-Cycle,-Period, -Fraction, -Zone, -Row.names)
  distMoni <- vegdist(dtMonix, method ="bray")
  PeriodsXMonimolimnion_anosim <- anosim(distMoni, dtMoni$Period, permutations = 9999, parallel = 10)
  capture.output(PeriodsXMonimolimnion_anosim,file="Betadisp/anosim_PeriodsinMonimolimnion_seq.txt")
#
## Betadisper
  # Betadisper
