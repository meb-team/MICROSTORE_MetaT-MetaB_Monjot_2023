...
# Input argument if using Rstudio
  if (inputmode == TRUE) {
  input <- "Taxonomic_Profil_Table_Final_5.csv"
...
#
# Prepare table -----------------------------------------------------------
  tableTraits <- tableVinput %>% select(Last,Size.min,Size.max,Cover,Shape,Spicule,Symmetry,Polarity,Colony,Motility,Plast.origin,Ingestion.mode,Symbiontic,Resting.stage,Suspected.trophism,IN.OUT)
...
## As numeric
  facteur=c("Size.min","Size.max","Cover","Shape","Spicule","Symmetry","Polarity","Colony","Motility","Plast.origin","Ingestion.mode","Symbiontic","Resting.stage","Suspected.trophism")
  tableTraits[,all_of(facteur)] <- lapply(tableTraits[,all_of(facteur)], as.character)
## Colony
  for (i in row.names(tableTraits)) {
    if (is.na(tableTraits[i,"Colony"])==FALSE) {
      if (tableTraits[i,"Colony"]=="None") { tableTraits[i,"Colony"] <- "No_Colony"}}
    if (is.na(tableTraits[i,"Spicule"])==FALSE) {
      if (tableTraits[i,"Spicule"]==0) { tableTraits[i,"Spicule"] <- "No_Spicule"}
      if (tableTraits[i,"Spicule"]==1) { tableTraits[i,"Spicule"] <- "Spicule"}}
    if (is.na(tableTraits[i,"Resting.stage"])==FALSE) { 
      if (tableTraits[i,"Resting.stage"]==0) { tableTraits[i,"Resting.stage"] <- "No_Resting_Stage"}
      if (tableTraits[i,"Resting.stage"]==1) { tableTraits[i,"Resting.stage"] <- "Resting_Stage"}}
    if (is.na(tableTraits[i,"Polarity"])==FALSE) { 
      if (tableTraits[i,"Polarity"]!="Heteropolar") { tableTraits[i,"Polarity"] <- "Isopolar"}}
    if (is.na(tableTraits[i,"Plast.origin"])==FALSE) { 
      if (tableTraits[i,"Plast.origin"]!="None") { tableTraits[i,"Plast.origin"] <- "Plast"}
      if (tableTraits[i,"Plast.origin"]=="None") { tableTraits[i,"Plast.origin"] <- "No_Plast"}}
    if (is.na(tableTraits[i,"Suspected.trophism"])==FALSE) { 
      if (tableTraits[i,"Suspected.trophism"]=="Parasite") { tableTraits[i,"Suspected.trophism"] <- "Heterotroph"}}
    if (is.na(tableTraits[i,"Ingestion.mode"])==FALSE) { 
      if (tableTraits[i,"Ingestion.mode"]=="No") { tableTraits[i,"Ingestion.mode"] <- "Osmotrophic_Or_No_Ingestion"}
      if (tableTraits[i,"Ingestion.mode"]=="Osmotrophic") { tableTraits[i,"Ingestion.mode"] <- "Osmotrophic_Or_No_Ingestion"}}
  }
  write.table(tableTraits, file = "Functional-Analyse/Group_FUNCTION/Table_Trait.csv",row.names = TRUE,sep=";")
...
#
  # Plot --------------------------------------------------------------------
    myplots <- list()
    i<-0
    for (factor in c("Size.min","Size.max","Cover","Shape","Spicule","Symmetry","Polarity","Colony","Motility","Plast.origin","Ingestion.mode","Symbiontic","Resting.stage","Suspected.trophism")) {
  # Dim1 and 2 -------------------------------------------------------------------
    i<-i+1
    print(noquote(factor))
    data <- data_seq_trait %>% filter(!is.na(eval(parse(text = factor))))
...
#
  ...
  # Group definition --------------------------------------------------------
    ...
    data_Grp <- data %>% filter(nbGroup==i)
    ...
    data_Grp_Plast.origin<- data_Grp %>% group_by(Value=Plast.origin) %>% summarise(n = n()) ; data_Grp_Plast.origin$Factor="Plast.origin"
    data_Grp_Symbiotic<- data_Grp %>% group_by(Value=Symbiontic) %>% summarise(n = n()) ; data_Grp_Symbiotic$Factor="Symbiotic"
...
#
  ...
  ## Format legend
    data_Grp_all$Factor[data_Grp_all$Factor=="Ingestion.mode"] <- "Ingestion"
    data_Grp_all$Factor[data_Grp_all$Factor=="Plast.origin"] <- "Plast Origin"
    data_Grp_all$Factor[data_Grp_all$Factor=="Resting.stage"] <- "Resting Stage"
    data_Grp_all$Factor[data_Grp_all$Factor=="Suspected.trophism"] <- "Expected Trophism"
  ## Plot
    svglite(paste0("Functional-Analyse/Group_FUNCTION/","Def_group_by_factor",".svg"),width = 20.00,height = 8.00)
    histGrpall <- ggplot(data_Grp_all, mapping = aes(y=n, x = Value, fill = Factor, group = Factor), Rowv = NA, col = colMain, scale = "column") + geom_bar(stat="identity") + 
      scale_fill_paletteer_d("ggthemes::manyeys") + 
      facet_grid(Group~Factor,scales="free") + theme_unique_dark()
    legendGRP <- get_legend(histGrpall)
    histGrpall <- histGrpall + theme(legend.position = "none") #+ labs(x="Cycles",y="ASVs (%)") 
    print(histGrpall)
    dev.off()
...
#
# Tree --------------------------------------------------------------------
  palet_tree_grp <- c("#607848FF","#d84040FF","#ad0635FF","#70a830FF","#60d0a0FF","#184068FF","#4080c0FF","#98d048FF","#40a078FF","#333b43FF","#2c7890FF","#b4d2d2")
