...
# Import package and palette -----------------------------------------------------------
  pkg <- c("ggplot2","readxl","dplyr","tidyr","cowplot","FactoMineR","factoextra","reshape2","varhandle","ggrepel","ggpubr","ggsci","scales","hrbrthemes","GUniFrac","svglite","stringr","paletteer","elementalist","SciViews","vegan","parallel")
...
# Diversity Raw ---------------------------------------------------------------
  ...
  ## Date
    ...
    adiv <- ggplot(Div_Table, aes(x = Date, y = Shannon)) + geom_boxplot() + geom_point(aes(color = Amplicon), size = 2.5) + 
      ...
      labs(x="Date",y="Diversity") #+ guides(color = FALSE)
    print(adiv)
...
#
  ...
  ## ZoneXPeriods
    ...
    aidiv <- ggplot(Div_Table, aes(x = Date, y = Pielou)) + geom_violin(aes(fill = Date),trim = FALSE,width=0.5,alpha=0.9) + geom_boxplot(width=0.075) + geom_jitter(position=position_jitter(0.05)) +
      stat_compare_means(method="wilcox.test", paired = FALSE, label = "p.signif", comparisons = my_comp) +
      stat_compare_means(method="wilcox.test", paired = FALSE, label = "p.signif", comparisons = my_comp) +
  ...
  ## ZoneXPeriods
    my_comp <- list(c("04","06"),c("04","09"),c("04","11"),c("06","09"),c("06","11"),c("09","11"))
    palet_periodcomp <- c("#7375b5FF","#8ca252FF","#f6ae34FF","#ad494aFF")
    bidiv <- ggplot(Div_Table, aes(x = Date, y = Shannon)) + geom_violin(aes(fill = Date),trim = FALSE,width=0.5,alpha=0.9) + geom_boxplot(width=0.075) + geom_jitter(position=position_jitter(0.05)) +
      stat_compare_means(method="wilcox.test", paired = FALSE, label = "p.signif", comparisons = my_comp) +
      stat_compare_means(method="wilcox.test", paired = FALSE, label = "p.signif", comparisons = my_comp) +
      facet_grid(~Zone) + scale_fill_manual(values = palet_periodcomp) + theme_unique_art() +
...
#
  ...
  ## ZoneXPeriods
    my_comp <- list(c("04","06"),c("04","09"),c("04","11"),c("06","09"),c("06","11"),c("09","11"))
    palet_periodcomp <- c("#7375b5FF","#8ca252FF","#f6ae34FF","#ad494aFF")
    zmdiv <- ggplot(Div_Table, aes(x = Date, y = Obs)) + geom_violin(aes(fill = Date),trim = FALSE,width=0.5,alpha=0.9) + geom_boxplot(width=0.075) + geom_jitter(position=position_jitter(0.05)) +
      stat_compare_means(method="wilcox.test", paired = FALSE, label = "p.signif", comparisons = my_comp) +
      #stat_compare_means(method="wilcox.test", paired = FALSE, label = "p.signif", comparisons = my_comp) +
      facet_grid(~Zone) + scale_fill_manual(values = palet_periodcomp) + theme_unique_art() +
      labs(x="Periods",y="Shannon index") #+ guides(color = FALSE)
